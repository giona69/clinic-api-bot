...
// Known excel fields
/*
  const ret = FIELDS.reduce((acc, f) => {
    acc[f] = slugify(f.replace(/\([^()]*\)/g, ''), { replacement: '_', lower: true });
    return acc;
  }, {});
*/
const FIELDS = {
  ...
  "Non-volatile Memory Capacity (MiB non-volatile)":
    "non_volatile_memory_capacity",
  ...
  "Secondary Image Sensor Pixel Size (micrometer)":
    "secondary_image_sensor_pixel_size",
  "Secondary Camera Number of pixels (MP sec. cam)":
    "secondary_camera_number_of_pixels",
...
const archiveFile = async (filename) => {
  ...
  await s3
    .copyObject({ Bucket: BUCKET, CopySource: source, Key: newName })
    .promise()
...
const syncDB = async () => {
  Utils.log("PHONEDB-LIB", "syncDB")

  const files = await listFiles()
  if (!files || files.length === 0) {
    Utils.log(`No new files found in bucket ${BUCKET}`)
    return {}
  }
