...
const syncPUBMEDTerm = async (termToSearch) => {
  ...
  const { count } = eSearch.esearchresult;
  // eslint-disable-next-line camelcase
  const author_count = count;
  // eslint-disable-next-line camelcase
  const investigator_count = 0;
...
const exportPubmed = async (offset) => {
  ...
  const usersList = XLSX.utils.sheet_to_json(ws).slice(0, 50);
  let index = 1;
  ...
  await usersList.forEachAsync(async (userTerm) => {
    ...
    try {
      ...
      Utils.logobj('CLINIC-DB-LIB', 'exportPubmed', error);
    }
