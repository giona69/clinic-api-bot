...
const syncTrialsDBData = async () => {
  ...
  await eSearch.FullStudiesResponse.FullStudies.forEachAsync(async (entry) => {
    try {
      ...
      if (
        // eslint-disable-next-line max-len
        entry.Study.ProtocolSection.ContactsLocationsModule.OverallOfficialList.OverallOfficial?.[0]?.OverallOfficialName
      ) {
        // eslint-disable-next-line max-len
        await entry.Study.ProtocolSection.ContactsLocationsModule.OverallOfficialList.OverallOfficial.forEachAsync(
          async (official) => {
            ...
            // eslint-disable-next-line max-len
            overallOfficialAffiliations += `${index === 0 ? '' : ' - '}${official.OverallOfficialAffiliation}`;
            index += 1;
          },
        );
      }
      ...
      await knex('study_entry').insert({
        ...
        // eslint-disable-next-line max-len
        completion_date: entry.Study.ProtocolSection.StatusModule?.CompletionDateStruct?.CompletionDate,
        ...
        // eslint-disable-next-line max-len
        interventions: entry.Study.ProtocolSection.ArmsInterventionsModule?.InterventionList?.Intervention?.[0].InterventionName,
        // eslint-disable-next-line max-len
        overall_official_names: overallOfficialNames,
        overall_official_affiliations: overallOfficialAffiliations,
        // eslint-disable-next-line max-len
        lead_sponsor_name: entry.Study.ProtocolSection.SponsorCollaboratorsModule?.LeadSponsor?.LeadSponsorName,
      });
    } catch (error) {
      Utils.errobj('CLINIC-DB-LIB', 'esearch error', error);
    }
